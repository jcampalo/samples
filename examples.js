...
function actualizaInvNueva() {
	var imp_invbiemu = 0;
  	var imp_invbiein = 0;
  	var valor = 0;
  	if ($("#imp_invbiemu").val().length < 1 && $("#imp_invbiein").val().length < 1){
    	$("#imp_invnueva").text("0,00");
    	return;
  	}
  	if ($("#imp_invbiemu").val().length > 0) {
    	imp_invbiein = desformateaImporte($("#imp_invbiemu").val());
    }
  	if ($("#imp_invbiein").val().length > 0) {
...
function checkPlazoMinimo(plazo,campoImporte) {
  	...
  	if ($(campoImporte).val().length > 0 && importe != 0) {
    	if ($(plazo).val().length > 0) {
      		var valor = parseInt($(plazo).val());
      		if ($(plazo).val() < 1) {
        		ataaMensaje('error','Plazo m�nimo','El plazo m�nimo es de 1 a�o','aceptar','void(0)','');
